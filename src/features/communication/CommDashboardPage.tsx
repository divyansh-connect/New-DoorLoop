...
import { MessageSquare, Mail, Phone, Bell, Send, Megaphone, ShieldAlert, Smartphone } from 'lucide-react';
...
export const CommDashboardPage: React.FC = () => {
  ...
  return (
    <div className="space-y-6 text-foreground">
      ...
      {/* QUICK ACTIONS BAR */}
      <div className="flex flex-wrap gap-2.5 p-3.5 bg-card border border-border rounded-2xl shadow-sm">
        <Button size="sm" onClick={() => navigate({ to: '/communication/inbox' })} className="flex items-center gap-1 font-semibold">
          <MessageSquare className="w-4 h-4" /> {t('commDashboard.openInbox')}
        </Button>
        <Button size="sm" variant="outline" onClick={() => navigate({ to: '/communication/email' })} className="flex items-center gap-1 font-semibold">
          <Mail className="w-4 h-4" /> {t('commDashboard.sendEmail')}
        </Button>
        <Button size="sm" variant="outline" onClick={() => navigate({ to: '/communication/sms' })} className="flex items-center gap-1 font-semibold">
          <Phone className="w-4 h-4" /> {t('commDashboard.sendSms')}
        </Button>
        <Button size="sm" variant="outline" onClick={() => navigate({ to: '/communication/announcements' })} className="flex items-center gap-1 font-semibold">
        ...
        <Button
          ...
          className="flex items-center gap-1.5 font-semibold text-rose-600 dark:text-rose-400 border-rose-500/30 hover:bg-rose-500/10"
        >
          <Mail className="w-4 h-4" /> Gmail / Outlook Add-on
        </Button>
        <Button
          size="sm"
          variant="outline"
          onClick={() => setIsPhoneModalOpen(true)}
          className="flex items-center gap-1.5 font-semibold text-blue-600 dark:text-blue-400 border-blue-500/30 hover:bg-blue-500/10"
        >
          <Smartphone className="w-4 h-4" /> SMS Deep Link (James' Phone)
        </Button>
      </div>
      ...
      <JamesPhoneSimulatorModal
        isOpen={isPhoneModalOpen}
        onClose={() => setIsPhoneModalOpen(false)}
        onNavigateToWorkOrder={() => navigate({ to: '/maintenance' })}
      />
    </div>
  );
};
